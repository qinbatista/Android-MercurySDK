	...
	public void repairindentRequest()//补单
	{
		...
		{		
			...
			public void run() 
			{
				...
				{
					LogLocalRestore("[E2WApp]->RepairindentRequest:E2WApp.DeviceId="+E2WApp.DeviceId+",url="+"http://101.200.214.15/pay/"+SdkApplication.channelname+"/"+QinConst.Restoreappid+"/add.php");
					...
					try 
					{
					   ...
					   // String tmp =  m.getMD5ofStr(E2WApp.DeviceId+"e2w");
					    ...
					    String log = HttpUrlpost("http://101.200.214.15/pay/"+SdkApplication.channelname+"/"+QinConst.Restoreappid+"/add.php",list);
	...
	public void respondCPserver(final String strID)//通知cp客户端完成订单
	{
		...
		new Thread(new Runnable() {		
			...
			public void run() {
				...
				String feedback =HttpUrlpost("http://101.200.214.15/pay/"+SdkApplication.channelname+"/"+QinConst.Restoreappid+"/respond.php",list);			
				LogLocalRestore("[InAppBase]->feedback:["+feedback+"]");
				...
				try 
				{
					...
					if(errorcodeString.equalsIgnoreCase("0"))
					{
						...
						LogLocalRestore("[InAppBase]->respondCPserver: code=0; PayList=["+readFileData("PayList")+"]");
					}
					else
					{
				...
				catch (JSONException e)
				{
					...
					e.printStackTrace();
					writeFileData("PayList",strID);
					LogLocalRestore("[InAppBase]->respondCPserver: code=error; PayList=["+readFileData("PayList")+"]");
				}
			}
