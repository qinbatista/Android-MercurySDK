...
public class InAppBaseStatistics {
	...
	{
		LogLocalRestore("[Rate]");
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("sku", sku);     //上线渠道名字
		map.put("channelname", channelname); //广告渠道名字
		map.put("uid", uid); //广告应用场景
		map.put("star", star); //广告类型
		map.put("feedback", feedback); //游戏名字
		map.put("version", version); //点击次数
		TalkingDataGA.onEvent("Rate", map);
	}
