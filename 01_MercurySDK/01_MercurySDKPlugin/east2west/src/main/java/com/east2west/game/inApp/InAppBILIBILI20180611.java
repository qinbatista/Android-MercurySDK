...
//endpublic class InAppBILIBILI20180611 extends InAppBase {

//comment	private InAppBase mBaseInApp = null;
	private String Channelname="InAppBILIBILI20180611";
	...
	public void init(Context appContext, Activity context, final String strAppId, final String strAppSecret,APPBaseInterface appinterface)
	{
		...
		new Handler(E2WApp.mContext.getMainLooper()).post(new Runnable() {
			@Override
			public void run() {
				E2WApp.LogLocal("["+Channelname+"] merchant_id="+merchant_id);
				E2WApp.LogLocal("["+Channelname+"] app_id="+app_id);
				E2WApp.LogLocal("["+Channelname+"] server_id="+server_id);
				E2WApp.LogLocal("["+Channelname+"] app_key="+app_key);
...
//								.show();
	...
	public void AnalysisID(String IDString)
	{
		try
		{
			...
			merchant_id=strArray[0].toString();
			server_id=strArray[1].toString();
			app_key=strArray[2].toString();
			secret_key=strArray[3].toString();
						...
						if(count==6&&QinConst.Restoreappid.equals("Machinarium"))
						{
							gameSdk.login(new CallbackListener() {
								...
								public void onSuccess(Bundle arg0) {
									...
									{
										E2WApp.LogLocal("["+Channelname+"] new user->userName="+userName+":uid="+uid);
										gameSdk.createRole(userName, uid);
										writeFileData("bilibili",uid);
									}
									else
									{
										E2WApp.LogLocal("["+Channelname+"] old user->userName="+userName+":uid="+uid);
									}
								...
								public void onFailed(BSGameSdkError arg0) {
									...
									E2WApp.LogLocal("["+Channelname+"]->init:InAppBase.onFailed");
//		        				makeToast("onFailed\nErrorCode : "
//		        						+ arg0.getErrorCode() + "\nErrorMessage : "
//		        						+ arg0.getErrorMessage());
									ShowTime();
								...
								public void onError(BSGameSdkError arg0) {
									...
									LogUtils.d("onError\nErrorCode : "
											+ arg0.getErrorCode() + "\nErrorMessage : "
											+ arg0.getErrorMessage());
									E2WApp.LogLocal("["+Channelname+"]->init:InAppBase.onError");
						...
						else
						{
							gameSdk.login(new CallbackListener() {
								...
								public void onSuccess(Bundle arg0) {
									...
									if(readFileData("bilibili")==""||!readFileData("bilibili").equals(uid))
									{
										E2WApp.LogLocal("["+Channelname+"] new user->userName="+userName+":uid="+uid);
										E2WApp.LogLocal("["+Channelname+"] readFileData(\"bilibili\")="+readFileData("bilibili"));
										gameSdk.createRole(userName, uid);
										writeFileData("bilibili",uid);
									}
									else
...
//						.show();
	...
	private void makeToast(String result) {
		...
		msg.what = InAppBILIBILI20180611.OK;
		msg.obj = result;
