...
//endpublic class InAppJRTT20181218 extends InAppBase {
	
//comment	private InAppBase mBaseInApp = null;
	private String Channelname="InAppJRTT20181218";
	private String mUid;
	...
	{		
		...
		E2WApp.LogLocal("["+Channelname+"] strAppKey111="+strAppId);
		E2WApp.LogLocal("["+Channelname+"] strAppSecret111="+strAppSecret);
		
		mBaseInApp = E2WApp.activityforappbase.getBaseInApp();
		TeaAgent.init(TeaConfigBuilder.create(mContext)
				.setAppName(QinConst.APPChineseName)
				.setChannel("east2west")
				.setAid(Integer.parseInt(strAppId))
				.createTeaConfig());
		TeaAgent.setDebug(true);
		//SsGameApi.init(mContext, clientId, payKey);
	}
	...
	public void onPause()
	{		
		E2WApp.LogLocal("["+Channelname+"] onPause="+mContext);
		try {
			TeaAgent.onPause(mContext);
		}catch (Exception e)
		{
			E2WApp.LogLocal("["+Channelname+"] e="+e);
		}
	}
	...
	{
		E2WApp.LogLocal("["+Channelname+"] onResume"+mContext);
		try {
