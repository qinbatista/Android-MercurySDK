...
public class InAppBaseOperation{
	...
	public void LoadConfiguration()//下载配置
	{
		new Thread()
		{
    		...
    		public void run()
    		{
    			if(SdkApplication.msg!=null)
    			{
	    			if(SdkApplication.msg.equals(""))
	    			{
	    				LogLocalSystem("[InAppBaseOperation][LoadConfiguration]Using default east2west setting");
	    				SdkApplication.msg="debug";
	    			}
				    ...
				    LogLocalSystem("[InAppBaseOperation][LoadConfiguration]http://101.200.214.15/setting/jsonServerSetting.php?name="+SdkApplication.msg+"Config"+"&appid="+QinConst.appid);
				    LogLocalSystem("[InAppBaseOperation][LoadConfiguration] Get Configuration Successfully");
				    if(!str.equals(""))
				    {
	...
	public boolean IsRatingAvailable()
	{
	    ...
	    try 
	    {
		    ...
		    String ratingSettings = Parameter1.getString("ratingSettings");	
		    
		    //LogLocalSystem("[InAppBaseOperation][IsRatingAvailable]url="+ratingSettings);
		    
		    JSONTokener jsonParserRAT = new JSONTokener(ratingSettings);  		    
		    JSONObject ParameterRAT; 
		    
	    	ParameterRAT = (JSONObject) jsonParserRAT.nextValue();
	    	String url = ParameterRAT.getString("ratingURL");	
		   // LogLocalSystem("[InAppBaseOperation][GetRatingURL]url="+url);	
	...
	{
		if(GetRemoteValue("ads_enabled").equals("true"))
			return true;
		else
			return false;
	}
    ...
    public static String HttpUrlpost(String URL)
    {
	 ...
	 {
		 try{
			 LogLocalSystem("[InAppBaseOperation][writeFileData]->writeFileData "+SdkApplication.Acontext);
			 FileOutputStream fout = SdkApplication.Acontext.openFileOutput(fileName, SdkApplication.Acontext.MODE_PRIVATE);
	 ...
	 public void writeNullData(String fileName)
	 {
		 String message="";
		 try{
			 //E2WApp.LogLocal("[E2WApp]->writeNullData fileName="+fileName+",message="+message+"-"+SdkApplication.Acontext);
			 FileOutputStream fout = SdkApplication.Acontext.openFileOutput(fileName, SdkApplication.Acontext.MODE_PRIVATE);
			 byte [] bytes = message.getBytes();
			 fout.write(bytes);
			 fout.close();	
	...
	public String readFileData(String fileName)
	{
	 		...
	 		try
	 		{
	 		  //E2WApp.LogLocal("[E2WApp]->readFileData:"+fileName+"-"+SdkApplication.Acontext);
	 		  FileInputStream fin = SdkApplication.Acontext.openFileInput(fileName);
